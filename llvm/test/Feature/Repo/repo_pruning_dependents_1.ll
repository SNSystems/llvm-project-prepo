...
; pass, only add a single RepoDefinition of C to the 'repo.tickets' in order to avoid multiple
...
; Step 3: Remove the RepoDefinition metadata from the function C.
; Step 4: Since the function C doesn't have the RepoDefinition, the digest of function C is generated
...
;         RepoDefinition of their dependent C is added to the 'repo.tickets'.
...
; RUN: %python %S/Inputs/remove_GO_repodefinition.py -r C -i %t.repo.ll -o %t.ll
