...
;
; If the digest of function B is generated by the RepoObjectWriter and a B TicketNode is
; also added during the pruning pass, the function B will use the existing TicketNode.
;
...
; Step 3: Remove the function B's TicketNode metadata from the %t.repo.ll.
; Step 4: Since the function B doesn't have the TicketNode, the digest of function B is generated
...
; Step 8: Remove the function B's TicketNode metadata from the %t.repo.2.ll.
...
;         TicketNode has been added during the pruning pass.
...
; RUN: %python %S/Inputs/remove_GO_ticketnode.py -r B -i %t.repo.ll -o %t.ll
...
; RUN: %python %S/Inputs/remove_GO_ticketnode.py -r B -i %t.repo.2.ll -o %t.2.ll
...
; CHECK: !TicketNode(name: "B", digest: [16 x i8] c"{{.*}}", linkage: external, visibility: default, pruned: true)
